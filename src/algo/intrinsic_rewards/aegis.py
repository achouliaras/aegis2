...
class DiscriminatorModel(IntrinsicRewardBaseModel):
    def __init__(
        ...
        self.dsc_local_obs_queue_len = dsc_obs_queue_len
        self.dsc_global_obs_queue_len = dsc_obs_queue_len # same length for local and global queues. It can be different
        self.global_rew_running_stats = RunningMeanStd(momentum=1.0) # CHECK maybe we need a custom class that has eviction policy
    ...
    def _get_fake_obs(self, curr_obs, next_obs, queue='local'):
        """
        ...
        """
        if queue == 'local':
            obs_queue = self.local_obs_queue
            obs_queue_filled = self.local_obs_queue_filled
            dsc_obs_queue_len = self.dsc_local_obs_queue_len
        elif queue == 'global':
            obs_queue = self.global_obs_queue
            obs_queue_filled = self.global_obs_queue_filled
            dsc_obs_queue_len = self.dsc_global_obs_queue_len
        else:
            raise ValueError("Queue type not recognized. Use 'local' or 'global'.")
        
        queue_len = min(obs_queue_filled, dsc_obs_queue_len)
        ...
        random_obs1 = obs_as_tensor(obs_queue[random_idx1], curr_obs.device)
        random_obs2 = obs_as_tensor(obs_queue[random_idx2], curr_obs.device)
    ...
    def _init_local_obs_queue(self):
        self.obs_shape = get_obs_shape(self.observation_space)
        self.local_obs_queue_filled = 0
        self.local_obs_queue_pos = 0
        self.local_obs_queue = np.zeros((self.dsc_local_obs_queue_len,) + self.obs_shape, dtype=float)


    def _init_global_obs_queue(self):
        self.obs_shape = get_obs_shape(self.observation_space)
        self.global_obs_queue_filled = 0
        self.global_obs_queue_pos = 0
        self.global_obs_queue = np.zeros((self.dsc_global_obs_queue_len,) + self.obs_shape, dtype=float)
    ...
    def _add_obs(self, obs, queue='local'):
        """
        Add one new element into the observation queue.
        """
        if queue == 'local':
            self.local_obs_queue[self.local_obs_queue_pos] = np.copy(obs)
            self.local_obs_queue_filled += 1
            self.local_obs_queue_pos += 1
            self.local_obs_queue_pos %= self.dsc_local_obs_queue_len
        elif queue == 'global':
            self.global_obs_queue[self.global_obs_queue_pos] = np.copy(obs)
            self.global_obs_queue_filled += 1
            self.global_obs_queue_pos += 1
            self.global_obs_queue_pos %= self.dsc_global_obs_queue_len


    def init_obs_queue(self, obs_arr, queue='local'):
        """
        ...
        """
        for obs in obs_arr:
            self._add_obs(obs, queue=queue)


    def update_obs_queue(self, iteration, intrinsic_rewards, local_ir_mean, new_obs, stats_logger):
        """
        ...
        for env_id in range(new_obs.shape[0]):
            if iteration == 0 or intrinsic_rewards[env_id] >= local_ir_mean:
                obs = new_obs[env_id]
                self._add_obs(obs, queue='local')
                stats_logger.add(local_obs_insertions=1)
            else:
                stats_logger.add(local_obs_insertions=0)
            
            if iteration == 0 or intrinsic_rewards[env_id] >= self.global_rew_running_stats.mean:
                # obs = new_obs[env_id] # Check what it does and if this is needes
                self._add_obs(obs, queue='global')
                stats_logger.add(global_obs_insertions=1)
            else:
                stats_logger.add(global_obs_insertions=0)


    def get_intrinsic_rewards(self,
        curr_obs: Tensor, next_obs: Tensor, last_mems: Tensor,
        obs_history: List, trj_history: List
    ):
        ...
        with th.no_grad():
            ...
            curr_dsc_losses, next_dsc_losses = self.forward


        # Create local IRs by the discriminator (Algorithm A1 in the Technical Appendix)
        batch_size = curr_obs.shape[0]
        ...
        for env_id in range(batch_size):
            ...
            # Apply NovelD in the global observation queue
            curr_novelty = curr_dsc_losses[env_id]
            next_novelty = next_dsc_losses[env_id]

            novelty = max(next_novelty - curr_novelty * self.noveld_alpha, self.noveld_beta)
            self.global_rew_running_stats.update(novelty)
    ...
    def optimize(self, rollout_data, stats_logger):
        ...
        with th.no_grad():
            ...
            fake_obs, obs_differences = self._get_fake_obs(curr_obs, next_obs, queue='local')
